...
def dfs_expression_builder(node, reachable, parent_dict, free_syms, cond_syms, cond, etype, ctype, inv=False):
	...
	if type(node).__name__ == "ExprComp":
		...
		if etype:
			...
			res0 = ANC([node.children[0]], [], node.children[0].depth, False).start()
			res1 = ANC([node.children[1]], [], node.children[1].depth, False).start()
...
def handleConditionals(probeNodeList, etype=True, inv=False):
	...
	# cstr = []
	# for probeNode in probeNodeList:
	# 	f_expr = probeNode.f_expression
	# 	if len(cstr):
	# 		if isinstance(f_expr, list):
	# 			newf_expr = []
	# 			for i in range(len(f_expr)):
	# 				if isinstance(f_expr[i][0], list):
	# 					for x in f_expr[i][0]:
	# 						newf_expr.append(str(x))
	# 						newf_expr.append(str(x))
	# 				else:
	# 					newf_expr.append(f_expr[i])
	#
	# 			cstr = [[x[0] + "&" + y[0]] for x in cstr for y in newf_expr]
	# 		else:
	# 			cstr = [[x[0] + "&" + f_expr] if isinstance(x, list) else [x + "&" + f_expr] for x in cstr]
	# 	else:
	# 		if isinstance(f_expr, list):
	# 			newf_expr = []
	# 			for i in range(len(f_expr)):
	# 				if isinstance(f_expr[i][0], list):
	# 					newf_expr.append(f_expr[i][0][0])
	# 					newf_expr.append(f_expr[i][0][1])
	# 				else:
	# 					newf_expr.append(f_expr[i])
	# 			cstr = newf_expr
	# 		else:
...
def writeToFile(results, fout, argList):
	...
	for outVar in Globals.outVars:
		...
		if Globals.domain_checks:
			dumpStr += "Domain check found these constraints to avoid infinity:\n"
